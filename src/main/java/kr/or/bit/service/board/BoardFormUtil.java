...
final class BoardFormUtil {
    ...
    static Integer getLoginEmpno(HttpServletRequest request) {
        ...
        if (session == null) {
            return TEMP_TEST_EMPNO;
        }

        Integer empno = toEmpno(session.getAttribute("empno"));
        if (empno != null) {
            return empno;
        }

        empno = toEmpno(session.getAttribute("loginEmpno"));
        if (empno != null) {
            return empno;
        }

        empno = toEmpno(session.getAttribute("loginEmp"));
        return empno == null ? TEMP_TEST_EMPNO : empno;
    }
