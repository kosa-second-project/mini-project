...
<main class="board-page">
    <c:choose>
        ...
        <c:when test="${board.deleted}">
            <h1>삭제된 게시글입니다.</h1>
            <div class="meta">글 번호 ${board.idx}</div>
            <jsp:include page="/WEB-INF/views/board/comment_section.jsp">
                <jsp:param name="boardIdx" value="${board.idx}" />
            </jsp:include>
            <a href="${pageContext.request.contextPath}/BoardList.do">목록</a>
        </c:when>
        <c:otherwise>
...
<c:if test="${not empty kakaoMapKey and not empty board.lat and not empty board.lng and not board.deleted}">
